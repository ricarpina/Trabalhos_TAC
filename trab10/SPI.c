...
float convert_temp(unsigned int num){ // este num tem 16 bits
	//para verificar se o bit + significativo é 1 ou 0

	unsigned int tmp = 0b1000000000000000;
	unsigned int tmp_2=0b0111111111111111;
	unsigned int tmp_3 = tmp & num;//mascara para extrair o bit + significativo
	//tmp_2=tmp_2>>15;
	unsigned int value = num & tmp_2;
	 // shift á esquerda, fiquei só com 13 bits
	float temp;
	
	if(tmp_3 == 0){//significa que é positivo
	value = num >> 3;	
	temp = (double)value * 0.0625; // 0.0625 corresponde a cada degrau da representaçao	
	}else{
		//fazer complemento para 2
		value=num;
		value = ~value; // complemento 1
		value = value >> 3;
		value = value + 1; //complemento 2
